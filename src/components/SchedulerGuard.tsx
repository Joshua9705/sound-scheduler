...
export default function SchedulerGuard({ children }: { children: React.ReactNode }) {
  const { isScheduler, login } = useAuth();
  const [pin, setPin] = useState("");
  const [error, setError] = useState("");
  const [loading, setLoading] = useState(false);

  if (isScheduler) return <>{children}</>;
  ...
  return (
    <div className="flex items-center justify-center min-h-[60vh]">
      <div className="bg-zinc-900/70 border border-zinc-800/50 rounded-2xl p-8 w-full max-w-sm space-y-6 text-center shadow-xl">
        ...
        <div>
          <h2 className="text-xl font-bold text-white">需要排班權限</h2>
          <p className="text-zinc-500 text-sm mt-2">請輸入排班人或管理員密碼</p>
        </div>
